...
export const useAvatarStorage = () => {
     ...
     */
    const uploadAvatar = async (dataUrl, userId) => {
        if (!dataUrl) {
        ...
        try {
            ...
            if (uploadError) {
                throw new Error(`Error al subir la imagen: ${uploadError && typeof uploadError === 'object' && 'message' in uploadError ? uploadError.message : String(uploadError)}`);
            }
