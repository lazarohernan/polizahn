...
/**
 * Composable para la gestión y verificación de permisos de usuario
 * Implementa la verificación en dos niveles recomendada:
 * 1. Primero verifica el rol en auth.user (app_metadata/user_metadata)
 * 2. Para permisos específicos, consulta la tabla permisos_de_acceso
 */
interface UserPermissions {
  polizas_view?: boolean;
  polizas_create?: boolean;
  polizas_edit?: boolean;
  polizas_delete?: boolean;
  clientes_view?: boolean;
  clientes_create?: boolean;
  clientes_edit?: boolean;
  clientes_delete?: boolean;
  aseguradoras_view?: boolean;
  aseguradoras_create?: boolean;
  aseguradoras_edit?: boolean;
  aseguradoras_delete?: boolean;
  sistema_configuracion?: boolean;
  corredurias_gestion?: boolean;
}
...
export function usePermissions() {
  ...
  const tecnicoPermissions: UserPermissions = {
    polizas_view: true,
  ...
  const canAccessModule = (moduleName: string): boolean => {
    ...
    const viewPermissionKey = `${moduleName}_view` as keyof UserPermissions;
  ...
  const hasPermissionSet = (permission: string) => {
    return userPermissions.value.has(permission);
  };

  const hasAnyPermissionSet = (permissions: string[]) => {
    return permissions.some(p => userPermissions.value.has(p));
  };

  const hasAllPermissionSet = (permissions: string[]) => {
